...
class TableList extends Component {
    ...
    if (this.props.user.permissions.includes(9)) {
      ...
      cols.push({
        ...
        render: (text, record) =>
          <div>
            {record.tracking == 1
              ? <Button type="primary" onClick={() => this.viewHistory(record)}>
                  View History
                </Button>
              : null}
  ...
  viewHistory = row => {
    this.props._updateDialog(true, {
      title: "History of the table " + row.tableName,
      content: <TableHistory table={row} />
    });
  };
    ...
    return (
      <div>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <Button onClick={() => this.deleteTable(tableInfo.key)}>Yes</Button>
        </div>
      </div>
    );
  };
  ...
  render() {
    if (
      !this.props.tables.tableList ||
      this.props.tables.tableList.length === 0
    ) {
      return (
        <div
          style={{
    ...
    return (
      ...
      >
        ...
        <Table
          ...
          footer={() =>
            this.props.user.permissions.includes(9) ? <AddTableButton /> : ""}
        />
      </div>
    );
  }
