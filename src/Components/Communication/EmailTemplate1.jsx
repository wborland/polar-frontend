...
class EmailTemplate1Component extends Component {
  constructor(props) {
    super(props);
    if (!props.user.auth) {
      props._push('/login');
    ...
    return (
      <div>
        <Form {...formItemLayout} onSubmit={this.handleSubmit} style={{ background: "#FFFFFF", padding: "10vh", textAlign: "center" }}>
          ...
          <Form.Item label="Body 3">
            {getFieldDecorator('bodyThree', {
              rules: [
                {
                  required: true,
                  message: 'Please enter a message',
...
const EmailTemplate1Form = Form.create({ name: 'EmailTemplate1' })(EmailTemplate1Component);
...
export default connect(mapStoreToProps, mapDispatchToProps)(EmailTemplate1Form);
