...
import { getEventById, getRsvpList } from "../../Redux/events";
...
class RsvpFormComponent extends Component {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        axios
          ...
          .then(response => {
            message.success(
              "You have Rsvp'd for " + decodeURI(this.props.currEvent.name)
            );
  ...
  render() {
    let formItems = [];
    if (
      this.props.currEvent.rsvp.questions &&
      this.props.currEvent.rsvp.questions.length !== 0
    ) {
      const { getFieldDecorator } = this.props.form;
      formItems = this.props.currEvent.rsvp.questions.map((val, index) =>
        <Form.Item required={false} key={index} label={val}>
          {getFieldDecorator("answers[" + index + "]", {
            initialValue:
              this.props.currEvent.rsvp.answers &&
              this.props.currEvent.rsvp.answers[index]
                ? this.props.currEvent.rsvp.answers[index]
                : "",
            rules: [
              {
