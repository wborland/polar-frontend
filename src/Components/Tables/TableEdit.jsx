...
import { getTableList, trackHistory, unTrackHistory } from "../../Redux/tables";
...
class TableEdit extends Component {
  ...
  handleDelete = item => {
    ...
    axios
      .post("/table/deleteColumn", reqBody)
      .then(response => {
  ...
  handleSave = item => {
    if (
      this.state.newVal == "" ||
      this.state.columns.includes(this.state.newVal)
    ) {
      ...
      axios
        .post("/table/modifyColumn", reqBody)
        .then(response => {
  ...
  handleTableNameSubmit = tableName => {
    if (tableName != "") {
      ...
      axios
        .post("/table/modifyTableName", reqBody)
        .then(response => {
  ...
  getTrackingInfo = () => {
    ...
    for (let i in list) {
      let curr = list[i];
      if (curr.key == this.props.table.key) {
        return curr.tracking;
      }
    }
  };
    ...
    return (
      <div>
        <Tabs defaultActiveKey="1" onChange={this.handleTabChange}>
            ...
            <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}>
              ...
              <Col lg={4}>
                <Button
                  type="primary"
                  onClick={() =>
                    this.handleTableNameSubmit(this.state.tableName)}
                >
                  Save
                </Button>
              </Col>
          ...
          <TabPane tab="Edit table history" key="4">
            {this.getTrackingInfo() == 1
              ? <Button
                  type="primary"
                  onClick={() => {
                    this.props._unTrackHistory(
...
const mapDispatchToProps = {
  ...
  _unTrackHistory: unTrackHistory
};
